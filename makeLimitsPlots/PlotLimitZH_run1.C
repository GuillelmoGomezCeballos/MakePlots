...
void DrawTLatex(Double_t x, Double_t y, Double_t tsize, const char* text, Short_t align = 32);

void PlotLimitZH_run1(string   limitFiles   = "inputs/ana_ICHEP_limits_nj_shape7teV_cut8TeV.txt",
	         ...
	         Int_t    setLogx      = 0,
	         Int_t    setLogy      = 1,
	         string   title0       = "H #rightarrow WW #rightarrow 2l2#nu",
	         string   title1       = "",
	         string   title2       = "",
		 ...
		 Double_t slope1       = 0.0)
{
  ...
  vector<float> vMedianExpLimit1;
  vector<float> vMedianExpLimit2;
  ...
  float ExpLim95Down; 
  float ExpLim95Up;
  float MedianExpLimit1; 
  float MedianExpLimit2; 
  ...
  while (indata
    ...
    vMedianExpLimit1.push_back(MedianExpLimit1); 
    vMedianExpLimit2.push_back(MedianExpLimit2); 
  }
  ...
  for (UInt_t i=0; i<npoints; ++i) {
    ...
    y1  [i] = vMedianExpLimit1.at(i);
    y2  [i] = vMedianExpLimit2.at(i);
  ...
  TGraph*            ExpLim1   = new TGraph           (npoints, x, y1);
  TGraph*            ExpLim2   = new TGraph	      (npoints, x, y2);
  if(y2[0] == -1) {
    float x_new   [npoints];
    float y2_new  [npoints];
    int npoints_new = 0;
    for (UInt_t i=0; i<npoints; ++i) {
      if(y2[i] > 0){
        x_new[npoints_new]  = vMass.at(i);
        y2_new[npoints_new] = vMedianExpLimit2.at(i);
        npoints_new++;
      }
    }
    ExpLim2 = new TGraph (npoints_new, x_new, y2_new);
  }
  ...
  if     (ratio != 6 && ratio != 7 && ratio != 9 && ratio != 10) ExpBand95->GetXaxis()->SetTitle("Higgs boson mass [GeV]");
  else if(ratio == 9)                                            ExpBand95->GetXaxis()->SetTitle("c#tau_{#tilde{#chi}_{1}^{0}} [mm]");
  else if(ratio == 10)                                           ExpBand95->GetXaxis()->SetTitle("#gamma_{D} mass [GeV]");
  else                                                           ExpBand95->GetXaxis()->SetTitle("#tilde{#chi}_{1}^{0} mass [GeV]");
  ...
  if (ratio == 3) ExpBand95->GetYaxis()->SetTitle("#sigma #Beta(H #rightarrow WW #rightarrow 2l2#nu) (pb)");
  if (ratio == 4) ExpBand95->GetYaxis()->SetTitle("#sigma #Beta(H #rightarrow invisible)/#sigma_{SM}");
  if (ratio == 5) ExpBand95->GetYaxis()->SetTitle("#sigma #Beta(H^{#pm#pm} #rightarrow W^{#pm}W^{#pm}) (fb)");
  //if (ratio == 6) ExpBand95->GetYaxis()->SetTitle("#sigma #Beta(H #rightarrow invisible + #geq 1#gamma)/#sigma_{SM}");
  //if (ratio == 7) ExpBand95->GetYaxis()->SetTitle("#sigma #Beta(H #rightarrow invisible + #geq 1#gamma) (fb)");
  //if (ratio == 8) ExpBand95->GetYaxis()->SetTitle("#sigma #Beta(H #rightarrow invisible + #geq 1#gamma) (fb)");
  if (ratio == 6 || ratio == 2 ||
      ratio == 9 || ratio == 10) ExpBand95->GetYaxis()->SetTitle("#sigma #Beta / #sigma_{SM}");
  if (ratio == 7) ExpBand95->GetYaxis()->SetTitle("#sigma #Beta (fb)");
  if (ratio == 8) ExpBand95->GetYaxis()->SetTitle("#sigma #Beta (fb)");
  ...
  ExpLim   ->Draw("l");
  if(y1[0] > 0){
    ExpLim1->SetLineStyle(6);
    ExpLim1->SetLineWidth(3);
    ExpLim1->SetLineColor(9);
    ExpLim2->SetLineStyle(8);
    ExpLim2->SetLineWidth(3);
    ExpLim2->SetLineColor(2);
    ExpLim1->Draw("l");
    ExpLim2->Draw("l");
  }
  ...
  if (canvas->GetLogy()) {
    ...
    ExpBand95->GetYaxis()->SetRangeUser(min*0.50, max+400);
  }
  else {
    ExpBand95->GetYaxis()->SetRangeUser(min-0.0, max+1);
    if      (ratio == 6 || ratio == 2 || ratio == 10) ExpBand95->GetYaxis()->SetRangeUser(min-0.0, max+0.4);
  ...
  DrawTLatex(0.53+slope0, 0.80, 0.032, title0.c_str(),12);
  DrawTLatex(0.95       , 0.94, 0.032, TString(luminosity).Data());
  if(strcmp(title1.c_str(),"") != 0)
    DrawTLatex(0.53+slope0, 0.75, 0.032, title1.c_str(),12);
  if(strcmp(title2.c_str(),"") != 0)
    DrawTLatex(0.53+slope0, 0.70, 0.032, title2.c_str(),12);

  TLegend* leg = new TLegend(0.15+slope1, 0.66, 0.355+slope1, 0.88, "");
  ...
  if(y1[0] > 0){
    leg->AddEntry(ExpLim1, " Median expected ZH",  "l");
    leg->AddEntry(ExpLim2, " Median expected ggH", "l");
  }
  leg->Draw("same");
...
// DrawTLatex
//------------------------------------------------------------------------------
void DrawTLatex(Double_t x, Double_t y, Double_t tsize, const char* text, Short_t align)
{
