...
void PlotLimit_run1(string  limitFiles   = "inputs/ana_ICHEP_limits_nj_shape7teV_cut8TeV.txt",
	       ...
	       string  format       = "pdf")
{
  ...
  for (UInt_t i=0; i<npoints; ++i) {
    ...
    if (y[i] + yu95[i] > max && x [i] <= mhmax) max = y[i] + yu95[i];
    if (y[i] - yd95[i] < min && x [i] >= mhmin) min = y[i] - yd95[i]; // 0.01;
  }
  ...
  if (ratio == 3) ExpBand95->GetYaxis()->SetTitle("#sigma X #Beta(H(inv.)) (pb)");
  if (ratio == 4) ExpBand95->GetYaxis()->SetTitle("#sigma X #Beta(H(inv.))/#sigma_{SM}");
  if (ratio == 5) ExpBand95->GetYaxis()->SetTitle("#sigma X #Beta(H(inv.)) (pb)");
  ...
  if (drawObserved) {
	...
	yObs[i] /= vMedianExpLimit.at(i);
      ...
      if (yObs[i] > max && x [i] <= mhmax) max = yObs[i];
      if (yObs[i] < min && x [i] >= mhmin) min = yObs[i];
    }
  ...
  //DrawTLatex(0.38, 0.94, 0.032, "#scale[1.2]{#font[62]{CMS}} #it{Preliminary}");
  DrawTLatex(0.21, 0.94, 0.032, "#scale[1.2]{#font[62]{CMS}}");
  DrawTLatex(0.94, 0.83, 0.040, title.c_str());
